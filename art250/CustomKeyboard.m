...
{    
    ...
    UISegmentedControl *doneNavigation = [[UISegmentedControl alloc] initWithItems:[NSArray arrayWithObjects:@"Done", nil]];
    [doneNavigation setEnabled:YES forSegmentAtIndex:0];
    doneNavigation.momentary = YES;
    
    CGRect doneNavigationFrame = doneNavigation.frame;
    doneNavigationFrame.size.width = doneNavigationFrame.size.width * 2;
    [doneNavigation setFrame:doneNavigationFrame];
    [doneNavigation addTarget:self action:@selector(userClickedDone:) forControlEvents:UIControlEventValueChanged];
    
    
    UIBarButtonItem *doneButton =[[UIBarButtonItem alloc] initWithCustomView:doneNavigation];
